report 50350 "SBC Sales Invoice MCP"
{
    ApplicationArea = All;
    Caption = 'SBC Sales Invoice MCP';
    UsageCategory = ReportsAndAnalysis;
    DefaultLayout = RDLC;
    RDLCLayout = '.\src\report\layout\SBCSalesInvoiceMCP.rdl';
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    dataitem(SalesInvLine; "Integer")
                    {
                        ...
                        column(TempSalesInvoiceLineDescription; GetTempLineDesc)
                        {
                        }
    ...
    local procedure GetTempLineDesc(): Text
    var
        GLSetup: Record "General Ledger Setup";
        DimensionValue: Record "Dimension Value";
        DescTxtBuilder: TextBuilder;
    begin
        PopulateTxtBuilder(DescTxtBuilder, TempSalesInvoiceLine.Description, false);
        GLSetup.GetRecordOnce();
        if TempSalesInvoiceLine."Shortcut Dimension 1 Code" <> '' then
            DimensionValue.Get(GLSetup."Global Dimension 1 Code", TempSalesInvoiceLine."Shortcut Dimension 1 Code");
        PopulateTxtBuilder(DescTxtBuilder, 'BRAND: ' + DimensionValue.Name, true);
        PopulateTxtBuilder(DescTxtBuilder, 'GRWT: ' + Format(TempSalesInvoiceLine."Gross Weight"), true);

        exit(DescTxtBuilder.ToText());
    end;

    local procedure PopulateTxtBuilder(var DescTxtBuilder: TextBuilder; TxtValue: Text; AddNewLine: Boolean)
    var
        Char10: Char;
    begin
        if AddNewLine then begin
            Char10 := 10;
            DescTxtBuilder.Append(Char10);
        end;
        DescTxtBuilder.Append(TxtValue);
    end;
}
