...
/// </summary>
report 50040 "SBC Export Purchase Orders"
{
    AdditionalSearchTerms = 'Export Purchase Orders';
    AllowScheduling = true;
    ApplicationArea = All;
    Caption = 'Export Purchase Orders to Excel';
    Description = 'Exports Purchase Orders to Excel based on the Export Definition Code set in the Export Options page.';
    ProcessingOnly = true;
    UsageCategory = Tasks;
    UseRequestPage = true;
    dataset
    {
        dataitem(PurchaseHeader; "Purchase Header")
        {
            ...
            dataitem(PurchaseLine; "Purchase Line")
            {
                ...
                begin
                    GlobalSBCOEExport.AddExportEntry(PurchaseLine."Document No.", PurchaseLine.SystemId, Database::"Purchase Line", "SBCOE Row Type"::Detail);
                end;
            }
            ...
            trigger OnAfterGetRecord()
            begin
                GlobalSBCOEExport.AddExportEntry(PurchaseHeader."No.", PurchaseHeader.SystemId, Database::"Purchase Header", "SBCOE Row Type"::Header);
            end;
        }
    }
    requestpage
    ...
    begin
        ...
        SBCOEExportWrapper.SetGlobalContactEmailList(GlobalContactEmailsList);
    ...
    internal procedure SetContactEmailList(ContactEmailsList: List of [Text])
    begin
    ...
    internal procedure SetExportMailGroupCode(ExportMailGroupCode: Code[20])
    begin
        if ExportMailGroupCode = '' then
            exit;
        GlobalMailGroupCode := ExportMailGroupCode;
    end;
    ...
    begin
        ...
        if GlobalMailGroupCode = '' then
            GlobalMailGroupCode := GlobalSBCOEExport.GetExportDefinition()."Email Group Code";
        GlobalSBCOEExport."Email Group Code" := GlobalMailGroupCode;
