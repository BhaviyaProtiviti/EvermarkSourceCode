...
/// </summary>
report 50204 "SBCTA Create Purch COGs"
{
    ApplicationArea = All;
    Caption = 'Create Purch. Indirect COGs Ledger Entries';
    ProcessingOnly = true;
    UsageCategory = Tasks;
    UseRequestPage = true;
    dataset
    {
        ...
        {
            DataItemTableView = where("Entry Type" = filter("Direct Cost" | "Revaluation"), "Item Ledger Entry Type" = filter(Purchase | "Negative Adjmt." | "Positive Adjmt."), "Document Type" = filter("Purchase Receipt" | "Purchase Return Shipment" | " "), "Source Type" = filter("Vendor" | " "));
