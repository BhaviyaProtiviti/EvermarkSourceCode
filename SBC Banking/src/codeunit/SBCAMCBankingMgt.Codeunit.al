...
codeunit 50603 "SBC AMC Banking Mgt."
{
    ...
    procedure GetCleanLicenseServerName(AMCBankingSetup: Record "AMC Banking Setup"): Text
    var
    ...
    procedure IsLicenseEqualAMC(): Boolean
    begin
        if (GetLicenseNumber() = CopyStr('BC' + AMCTenantId(), 1, 40)) then
            exit(true)
        else
    ...
    procedure GetCurrentApiVersion(): Text[10]
    begin
        exit(ApiVersionTxt);
    end;
    ...
    procedure IsAMCFundamentalsEnabled(): Boolean
    var
        AMCBankingSetup: Record "AMC Banking Setup";
    begin
        if (not AMCBankingSetup.get()) then
    ...
    procedure IsAMCBusinessInstalled(): Boolean
    var
        AppInfo: ModuleInfo;
    begin
    ...
    procedure SetFieldValue(var RecordRef: RecordRef; FieldId: Integer; XmlValueVariant: Variant; AppendToValue: Boolean; UseValidate: Boolean)
    var
    ...
    procedure updateValue(var fieldref: FieldRef; ValueVariant: Variant; UseValidate: Boolean)
    var
    begin
        if (UseValidate) then
    ...
    procedure GetFieldValue(RecordRef: RecordRef; FieldNo: Integer): Text;
    var
        TypeHelper: Codeunit "Type Helper";
        FieldRef: FieldRef;
        DateVariant: Variant;
        ...
        case FieldRef.Type of
            ...
            FieldType::Date:
                begin
                    ...
                    TransformedValue := FORMAT(DateTimeValue, 0, '<Year4>-<Month,2>-<Day,2>T<Hours24>:<Minutes,2>:<Seconds,2>');
                end;
            FieldType::Decimal:
                TransformedValue := FORMAT(FieldRef.Value(), 0, 9);
            FieldType::Integer:
