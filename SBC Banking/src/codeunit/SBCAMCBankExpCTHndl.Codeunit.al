...
codeunit 50600 "SBC AMC Bank Exp. CT Hndl"
{
    ...
    begin
        ...
        if (BankAccount.Get(PayBankAcountNo)) then
            BankFileName := GetBankFileName(BankAccount)
        else
    ...
    begin
        ...
        SetUsedXTLJournal(PaymentFileTempBlob, DataExchEntryNo, PaymentExportWebCallTxt);
    ...
    begin
        ...
        if (Found) then begin
            ...
            BodyContentXmlDoc.WriteTo(DataText);
            if confirm('%1', false, DataText) then;
            AMCBankServiceRequestMgt.CreateEnvelope(EnvelopeXmlDoc, EnvelopeXmlElement, GetUserName(), GetPassword(), '');
    ...
    //     File.WriteAllBytes(FileName, Convert.FromBase64(Base64String));
    ...
    procedure GetBankFileName(BankAccount: Record "Bank Account"): Text[250]
    var
    ...
    procedure SetUsedXTLJournal(var TempBlob: Codeunit "Temp Blob"; DataExchEntryNo: Integer; PaymentExportWebCallTxt: Text)
    var
    ...
    begin
        AMCBankingSetup.get;
        if (AMCBankingSetup."User Name" = '') then
            exit(AMCBankingSetup.GetDemoUserName());

        ServiceUserName := AMCBankingSetup."User Name";
    ...
    begin
        AMCBankingSetup.get;
        if (AMCBankingSetup."User Name" = AMCBankingSetup.GetDemoUserName()) then
            exit(GetDemoPass());

        if not IsolatedStorage.Get(CopyStr(AMCBankingSetup."Password Key", 1, 200), Datascope::Company, Value) then;
        exit(Value);
    end;
